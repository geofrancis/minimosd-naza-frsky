...
void sendMavlink(uint8_t idx)
{
  ...
  // Send mavlink messge
  MAVLINK_PORT.write(MAVLINK_START_OF_FRAME);
  MAVLINK_PORT.write(mavlinkMsg, msgLen);
  MAVLINK_PORT.write(mavlinkCrc, MAVLINK_CHECKSUM_LEN);
}
...
void sendMavlinkGpsRawInt()
{
  ...
  encodeLong(8,   (int32_t)(GPS_DECODER.getLat() * 10000000));
  encodeLong(12,  (int32_t)(GPS_DECODER.getLon() * 10000000));
  encodeLong(16,  (int32_t)(GPS_DECODER.ALTITUDE_FUN() * 1000));
  encodeShort(20, (uint16_t)0xFFFF);
  encodeShort(22, (uint16_t)0xFFFF);
  encodeShort(24, (uint16_t)(GPS_DECODER.getSpeed()));
  encodeShort(26, (uint16_t)GPS_DECODER.HEADING_FUN());
  encodeByte(28,  GPS_DECODER.getFixType());
  encodeByte(29,  GPS_DECODER.getNumSat());
...
void sendMavlinkAttitude()
{
  ...
  encodeFloat(4,  (float)(GPS_DECODER.getRoll() * M_PI / 180));
  encodeFloat(8,  (float)(GPS_DECODER.getPitch() * M_PI / 180));
...
{
  encodeFloat(0, 0.0);
  encodeFloat(4,  (float)GPS_DECODER.getSpeed());
  encodeFloat(8,  (float)GPS_DECODER.ALTITUDE_FUN());
  encodeFloat(12, -(uint16_t)GPS_DECODER.VSI_FUN());
  encodeShort(16, (uint16_t)GPS_DECODER.HEADING_FUN());
  encodeShort(18, 0);
...
void setup()
{
  MAVLINK_PORT.begin(57600);
  GPS_PORT.begin(115200);
  FRSKY_TELEM_PORT.begin(57600);
}

void loop()
{
  if(GPS_PORT.available()) GPS_DECODER.decode(GPS_PORT.read());
